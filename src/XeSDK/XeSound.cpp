...
namespace Xe { namespace Sound {
	...
	inline float ConvertS24L(s8 *n, int i) {
		u8 *pn = (u8*)&n[i * 3];
		int value = (pn[0] | (pn[1] << 8) | (pn[2] << 16)) << 8;
		value >>= 8;
	...
	IAudioBuffer::IAudioBuffer(const WaveDesc &format) :
		m_format(format)
	{
	}
	...
	bool Create(IAudio **audio, ctstring driverName)
	{
		...
		Xe::Drivers::ISoundDriver* soundDriver = nullptr;
		auto drivers = Xe::Drivers::GetDrivers<Xe::Drivers::ISoundDriver>();
		for (auto it = drivers.begin(); it != drivers.end(); it++)
		{
			auto currentDriverName = (*it)->GetDriverName();
			if (strcmp(currentDriverName, driverName) == 0)
			{
				soundDriver = (Xe::Drivers::ISoundDriver*)*it;
			}
		}
