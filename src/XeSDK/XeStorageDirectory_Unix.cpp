...
#if defined(PLATFORM_UNIX) || defined(PLATFORM_ANDROID) || defined(PLATFORM_IOS)
...
Xe::RESULT Xe::Storage::Directory::Rename(ctstring path, ctstring newpath) {
	if (!Exists(path))
		return Error::IO_NOT_FOUND;
	if (Exists(newpath))
...
Xe::RESULT Xe::Storage::Directory::Open(IDirectory **directory, ctstring path) {
	*directory = (IDirectory*)nullptr;
	if (IsValid(path, String::GetLength(path)))
