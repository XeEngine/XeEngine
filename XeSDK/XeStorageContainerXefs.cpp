		...
		namespace Container {
			...
			class CXefs : public Xefs {
				...
				svar ObtainIndexFromFilename(ctstring filename) const {
					...
					/* da notare che il file system è case-sensitive, ma
					 * i percorsi in UNICODE verranno ritrasportati in
					 * UTF-8 se PLATFORM_UNICODE è definito.
					 */
					...
					/* se Content_Hash non è specificato, significa che la
					 * tabella degli hash non è pre-calcolata all'interno del
					 ...
					 * Richiamare HashSearchBinary solo se m_hash è un array
