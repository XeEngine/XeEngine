...
namespace Xe { namespace Crypto {
	static const u64 Crc64DefaultTable[] =
	{
		...
		0xa0dda7c5f3c4488eULL, 0xb9f3eb7bf06317e1ULL, 0xfe5991925b84e8d5ULL, 0x4d77dd2c5823b7baULL,
	...
	constexpr u64 Crc64_c(const char data[], u64 hash, int index, int remains) {
		return remains == 0 ? hash ^ -1 :
			Crc64_c(data, Crc64DefaultTable[(u8)hash ^ data[index]] ^ (hash >> 8), index + 1, remains - 1);
	}

	template <int length>
	constexpr u64 Crc64_c(const char(&data)[length]) {
		return Crc64_c(data, -1, 0, length - 1);
	}

	constexpr u64 Crc64_c(const char data[], int length) {
		return Crc64_c(data, -1, 0, length);
	}
