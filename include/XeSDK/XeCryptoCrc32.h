...
namespace Xe { namespace Crypto {
	static const u32 Crc32DefaultTable[] =
	{
	...
	constexpr u32 Crc32_c(const char data[], u32 hash, int index, int remains) {
		return remains == 0 ? hash ^ -1 :
			Crc32_c(data, Crc32DefaultTable[(u8)hash ^ data[index]] ^ (hash >> 8), index + 1, remains - 1);
	}

	template <int length>
	constexpr u32 Crc32_c(const char(&data)[length]) {
		return Crc32_c(data, -1, 0, length - 1);
	}

	constexpr u32 Crc32_c(const char data[], int length) {
		return Crc32_c(data, -1, 0, length);
	}
