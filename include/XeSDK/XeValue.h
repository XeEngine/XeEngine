...
namespace Xe {
    ...
    template <class T>
    struct ValueInteger : public Value<T> {
        ValueInteger<T>() : Value<T>(0) {}
        ValueInteger<T>(const T& value) : Value<T>(value) {}
        ...
        //ValueInteger<T> operator >> (const T& value) const { return ValueInteger<T>(m_value >> value); }
        //ValueInteger<T> operator << (const T& value) const { return ValueInteger<T>(m_value << value); }
        ValueInteger<T> operator ~() const { return ValueInteger<T>(~this->m_value); }
        ValueInteger<T> operator !() const { return ValueInteger<T>(!this->m_value); }
        ValueInteger<T> operator &&(ValueInteger<T> value) const { return ValueInteger<T>(this->m_value && value); }
        ValueInteger<T> operator ||(ValueInteger<T> value) const { return ValueInteger<T>(this->m_value || value); }
        ValueInteger<T>& operator %= (const T& value) { this->m_value %= value; return *this; }
        ValueInteger<T>& operator &= (const T& value) { this->m_value &= value; return *this; }
        ValueInteger<T>& operator |= (const T& value) { this->m_value |= value; return *this; }
        ValueInteger<T>& operator ^= (const T& value) { this->m_value ^= value; return *this; }
        ValueInteger<T>& operator >>= (const T& value) { this->m_value >>= value; return *this; }
        ValueInteger<T>& operator <<= (const T& value) { this->m_value <<= value; return *this; }
        bool TestBit(T bit) const { return !!(this->m_value & (1 << bit)); }
        void ResetBit(T bit) { this->m_value &= ~(1 << bit); }
        void SetBit(T bit) { this->m_value |= 1 << bit; }
        bool EqualBits(T bits) const { return (this->m_value & bits) == bits; }
    };
